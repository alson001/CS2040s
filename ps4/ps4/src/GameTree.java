...
public class GameTree {
    ...
    int findValue() {
        // TODO: Implement this
        return helper(this.root, Player.ONE);
    }
    int helper(TreeNode node, Player player) {
        if (node.leaf) {
            return node.value;
        }
        node.value = helper(node.children[0], other(player));
        for (int i = 1; i < node.numChildren; i++) {
            if (player == Player.ONE) {
                node.value = Math.max(helper(node.children[i], other(player)), node.value);
            }
            else if (player == Player.TWO) {
                node.value = Math.min(helper(node.children[i], other(player)), node.value);
            }
        }
        return node.value;
    }
