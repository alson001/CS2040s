/**
 * ScapeGoat Tree class
 *
 * This class contains some of the basic code for implementing a ScapeGoat tree.
 * This version does not include any of the functionality for choosing which node
 * to scapegoat.  It includes only code for inserting a node, and the code for rebuilding
 * a subtree.
 */

public class SGTree {

    // Designates which child in a binary tree
    enum Child {LEFT, RIGHT}

    /**
     * TreeNode class.
     *
     * This class holds the data for a node in a binary tree.
     *
     * Note: we have made things public here to facilitate problem set grading/testing.
     * In general, making everything public like this is a bad idea!
     *
     */
    public static class TreeNode {
        int key;
        int weight;
        public TreeNode left = null;
        public TreeNode right = null;

        TreeNode(int k) {
            key = k;
        }
    }

    // Root of the binary tree
    public TreeNode root = null;

    /**
     * Count the number of nodes in the specified subtree
     *
     * @param node  the parent node, not to be counted
     * @param child the specified subtree
     * @return number of nodes
     */
    public int countNodes(TreeNode node, Child child) {
        // TODO: Implement this
        if (child == Child.LEFT) {
            return helper1(node.left);
        }
        else if (child == Child.RIGHT) {
            return helper1(node.right);
        }
        return  0;
    }
    private int helper1(TreeNode node) {
        if (node == null) {
            return 0;
        }
        return 1 + helper1(node.left) + helper1(node.right);
    }

    /**
     * Build an array of nodes in the specified subtree.
     *
     * @param node  the parent node, not to be included in returned array
     * @param child the specified subtree
     * @return array of nodes
     */
    public TreeNode[] enumerateNodes(TreeNode node, Child child) {
        // TODO: Implement this
        int count = countNodes(node, child);
        TreeNode[] temp = new TreeNode[count];
        counter = 0;
        if (child == Child.LEFT) {
            inorderTraversal(node.left, temp);
        }
        else if (child == Child.RIGHT) {
            inorderTraversal(node.right, temp);
        }
        return temp;
    }

    private int counter = 0;
    private void inorderTraversal(TreeNode node, TreeNode[] temp) {
        if (node == null) {
            return ;
        }
        inorderTraversal(node.left, temp);
        temp[counter] = node;
        counter++;
        inorderTraversal(node.right, temp);
    }

    /**
     * Builds a tree from the list of nodes Returns the node that is the new root of the subtree
     *
     * @param nodeList ordered array of nodes
     * @return the new root node
     */
    public TreeNode buildTree(TreeNode[] nodeList) {
        // TODO: Implement this
        int start = 0;
        int end = nodeList.length - 1;
        return helper3(nodeList, start, end);
    }
    private TreeNode helper3(TreeNode[] nodeList, int start, int end) {
        if (start > end) {
            return null;
        }
        int mid = start + (end - start)/2;
        TreeNode node = nodeList[mid];
        node.left = helper3(nodeList, start, mid - 1);
        node.right = helper3(nodeList, mid + 1, end);
        if (node.left != null && node.right != null) {
            node.weight = node.right.weight + node.left.weight + 1;
        } else if (node.left == null && node.right != null) {
            node.weight = node.right.weight + 1;
        } else if (node.left != null && node.right == null) {
            node.weight = node.left.weight + 1;
        } else {
            node.weight = 1;
        }
        return node;
    }

    /**
     * Determines if a node is balanced.  If the node is balanced, this should return true.  Otherwise, it should return false.
     * A node is unbalanced if either of its children has weight greather than 2/3 of its weight.
     *
     * @param node a node to check balance on
     * @return true if the node is balanced, false otherwise
     */
    public boolean checkBalance(TreeNode node) {
        // TODO: Implement this
        if (node == null) {
            return true;
        } else if (checkWeight(node.left) > 2.0/3.0 * (checkWeight(node.left) + checkWeight(node.right) + 1.0)) {
            return false;
        } else if (checkWeight(node.right) > 2.0/3.0 * (checkWeight(node.left) + checkWeight(node.right) + 1.0)) {
            return false;
        } else {
            return true;
        }
    }

    public double checkWeight(TreeNode node) {
        if (node == null) {
            return 0.0;
        }
        return checkWeight(node.left) + checkWeight(node.right) + 1.0;
    }

    /**
     * Rebuild the specified subtree of a node.
     *
     * @param node the part of the subtree to rebuild
     * @param child specifies which child is the root of the subtree to rebuild
     */
    public void rebuild(TreeNode node, Child child) {
        // Error checking: cannot rebuild null tree
        if (node == null) return;
        // First, retrieve a list of all the nodes of the subtree rooted at child
        TreeNode[] nodeList = enumerateNodes(node, child);
        // Then, build a new subtree from that list
        TreeNode newChild = buildTree(nodeList);
        // Finally, replace the specified child with the new subtree
        if (child == Child.LEFT) {
            node.left = newChild;
        } else if (child == Child.RIGHT) {
            node.right = newChild;
        }
    }


    /**
     * Insert a key into the tree
     *
     * @param key the key to insert
     */
    public void insert(int key) {
        if (root == null) {
            root = new TreeNode(key);
            root.weight = 1;
            return;
        }

        TreeNode node = root;

        while (true) {
            if (key <= node.key) {
                if (node.left == null) {
                    node.weight++;
                    break;
                }
                node.weight++;
                node = node.left;
            } else {
                if (node.right == null) {
                    node.weight++;
                    break;
                }
                node.weight++;
                node = node.right;
            }
        }

        if (key <= node.key) {
            node.left = new TreeNode(key);
        } else {
            node.right = new TreeNode(key);
        }

        TreeNode temp = root;
        while (temp != null) {
            if (!checkBalance(temp.right)) {
                rebuild(temp, Child.RIGHT);
            } else if (!checkBalance(temp.left)) {
                rebuild(temp, Child.LEFT);
            }
            if (key <= temp.key) {
                temp = temp.left;
            } else {
                temp = temp.right;
            }
        }

    }


    // Simple main function for debugging purposes
    public static void main(String[] args) {
        SGTree tree = new SGTree();
        for (int i = 0; i < 100; i++) {
            tree.insert(i);
        }
        tree.rebuild(tree.root, Child.RIGHT);
        System.out.println(tree.root.weight);
    }
}
